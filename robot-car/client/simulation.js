...
function connectWebSocket() {
    ...
    try {
        ...
        game.ws.onopen = () => {
            ...
            // Register as untrusted client (to receive broadcasts)
            game.ws.send(JSON.stringify({
                type: "hello",
                device_id: "robot_car_sim_viewer",
                client_type: "untrusted"
            }));
        };
        ...
        game.ws.onmessage = (event) => {
            try {
                ...
                if (msg.type === 'connection') {
                    ...
                    // Update ciphertext preview with REAL encrypted data from server
                    game.lastCiphertext = JSON.stringify(msg.data).substring(0, 100);
...
async function transmitSensorData() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Ciphertext display is now updated via WebSocket broadcast
        }
    } catch (e) {
