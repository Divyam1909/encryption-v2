...
async def receive_sensor_data(packet: SensorDataPacket):
    ...
    if packet.encrypted:
        # Decrypt for trusted clients
        decrypted = server.decrypt_for_trusted(packet.sensor_data)
    else:
        # Already plaintext
        decrypted = packet.sensor_data
    
    # Broadcast to connected clients
    await server.broadcast_to_clients(packet.sensor_data, decrypted, packet.device_id)
