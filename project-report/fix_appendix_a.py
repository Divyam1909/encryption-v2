"""
Fix Appendix A: Remove the reversed/bad insertion and re-insert correctly.
"""
import sys
...
CODE_A1 = [
"# core/fhe_engine.py  -  CKKS Homomorphic Encryption Engine (SmartGridFHE)",
...
"        self.context.auto_rescale = self.context.auto_relin = True",
"",
...
"",
"    def encrypt_demand(self, demand_kw, agent_id) -> 'EncryptedDemand':",
...
"",
"    def aggregate_demands(self, enc_list) -> 'EncryptedDemand':",
...
"                               hashlib.sha256(ct).hexdigest()[:12])",
"",
"    def compute_average(self, enc_total, count) -> 'EncryptedDemand':",
...
"# Adaptive Linear Threshold (ALT): approximate comparison, zero ct*ct mult",
"import tenseal as ts, hashlib",
"from datetime import datetime",
"",
...
"        delta     = threshold / k            # soft-zone half-width (d = T/k)",
"        slope     = 0.5 / delta              # 0.5k / T",
"        intercept = 0.5 - threshold * slope  # 0.5 - 0.5k",
"",
"        # Core HE ops: multiply_plain then add_plain (depth 0)",
...
"            hashlib.sha256(b'SmartGridHE_Pedersen_Commitment_Generator_H_v1'",
"            ).digest(), 'big'), PRIME)",
"",
...
"        # C = g^m * h^r mod p  (hiding + binding + additive homomorphic)",
...
"        return C, r   # C -> coordinator (public); r -> utility (secret)",
"",
"    def aggregate(self, commitments: list) -> int:",
"        # C_agg = prod(C_i) = g^(sum_m) * h^(sum_r) mod p",
"        agg = 1",
...
def ppr(align="left", before=0, after=80, line=240, rule="auto"):
...
def para(text, bold=False, font="Calibri", size=11, align="left",
    ...
    p = OxmlElement('w:p')
    p.append(ppr(align, before, after))
    if text:
        r = OxmlElement('w:r')
...
def code_line(text):
    p = OxmlElement('w:p')
    p.append(ppr("left", before=0, after=0, line=160, rule="exact"))
    r = OxmlElement('w:r')
...
def main():
    ...
    # 1. Find the "Agnel Charities" paragraph (anchor)
    agnel_elem = None
    ...
    if agnel_elem is None:
        print("ERROR: Agnel paragraph not found!"); return

    # 2. Remove ALL existing bad Appendix A elements (everything between
    #    References end and Agnel paragraph).
    #    Strategy: walk backwards from agnel_elem; remove paragraphs that
    #    are NOT part of the original document (i.e., that were inserted
    #    by the previous bad run).  We stop when we hit the References section.

    removed = 0
    cursor = agnel_elem.getprevious()
    while cursor is not None:
        texts = ''.join(t.text for t in cursor.findall('.//' + qn('w:t')) if t.text)
        # Stop when we reach the last reference entry
        if '[22]' in texts or '[21]' in texts or '[20]' in texts:
            break
    ...
    print(f"Removed {removed} bad paragraphs")

    # 3. Build correct Appendix A elements in the right order
    elements = []
    ...
    # 4. Insert elements before agnel_elem
    #    NOT reversed - insert in order, each before agnel (last one lands closest to agnel)
    ...
    for i, p in enumerate(paras2):
        ...
        if 'APPENDIX A' in txt.upper() and app_a_idx is None:
            app_a_idx = i
        if 'Agnel' in txt and agnel_idx is None:
            agnel_idx = i
    print(f"Appendix A heading at para index: {app_a_idx}")
    print(f"Agnel paragraph at para index:    {agnel_idx}")
    print(f"Total paragraphs: {len(paras2)}")
    if app_a_idx is not None and agnel_idx is not None and app_a_idx < agnel_idx:
        print("SUCCESS: Appendix A appears before Agnel Charities page.")
    else:
        print("WARNING: Order check failed - inspect manually.")
